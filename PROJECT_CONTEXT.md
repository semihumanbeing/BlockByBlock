...
- **2번 장비 등록 & 설정 화면 (`EquipmentSetupScreen.kt`)**:
  ...
  - **몰드 규격 프리셋 & 칸 수 커스텀 (모바일 한 줄 최적화 3개 프리셋 + [직접] 버튼 + [↺ 원래 모양] 복구 버튼)**:
    - `500ml (ML_500)`: 1, 2, 4구 (기본 모양: 2구) + `[직접]`
    - `250ml (ML_250)`: 2, 4, 8구 (기본 모양: 4구) + `[직접]`
    - `125ml (ML_125)`: 4, 6, 8구 (기본 모양: 6구) + `[직접]`
    - `30ml (ML_30)`: 8, 16, 24구 (기본 모양: 16구) + `[직접]`
    - `직접 입력(CUSTOM)`: 6, 8, 16구 + `[직접]` (용량 ml 직접 입력)
    - **프리셋 몰드 모양 원상복구 버튼 (`[↺ 원래 모양]`)**:
      - 표준 프리셋 몰드(`ML_500`, `ML_250`, `ML_125`, `ML_30`)의 칸 수를 기본 모양과 다르게 변경했을 때만 나타나는 원터치 복구 버튼
      - `EquipmentSetupScreen`(좌측 몰드 뷰 하단 및 우측 칸 수 선택 행) 및 `SingleMoldEditDialog`(좌측 몰드 뷰 하단 및 칸 수 선택 행 좌측)에 제공
      - 탭 시 해당 프리셋의 공장 출하 기본 칸 수(`defaultCellCount`)로 즉시 복원되고 버튼은 자동 숨김
      - 다국어(i18n) 지원: 한국어 `"원래 모양"`, 영어 `"Reset Shape"`
    - **`CustomSlotChip` 동작**: 평소에는 `[직접]`(또는 비프리셋 선택 시 `[N구]`) 버튼으로 노출되며, 클릭 시 즉시 숫자 입력창으로 전환 및 가상 숫자 키패드(`KeyboardType.Number`) 자동 팝업
  - **조리기구 원터치 선택**:
...
- **3-State 순환 원터치 체크리스트 (`onToggleChecklistStatus`)**:
  ...
  - **`[소진됨 💨]`** 빈 원(○) 클릭 ➔ **`[장바구니 🛒]`**로 전환 ("사러 가요!")
  - **`[장바구니 🛒]`** 장바구니 로고(🛒) 클릭 ➔ **`[보유중 🟢]`**으로 복귀 ("장보기 완료!")
...
- **2. 끼니별 식단 기록 모달 (`MealRecordDialog.kt`)**:
  - **위쪽 (먹을 블록 영역 - Bento Lunchbox Tray)**:
    - 텍스트 없이 **오직 위에서 본 레고 블록 모양(`FoodBlockTopView`)**이 도시락 통(`LunchBoxView`) 안에 가로로 층층이 착착착 이어붙어 쌓임
