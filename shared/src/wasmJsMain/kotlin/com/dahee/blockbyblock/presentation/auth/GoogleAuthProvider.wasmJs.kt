...
""")
private external fun wasmEnsureJsHelper()

@OptIn(kotlin.js.ExperimentalWasmJsInterop::class)
@JsFun("(clientId) => { if (window.__startGoogleSignIn) window.__startGoogleSignIn(clientId); }")
private external fun wasmStartGoogleSignIn(clientId: String)

@OptIn(kotlin.js.ExperimentalWasmJsInterop::class)
@JsFun("() => { return (window.__getGoogleSignInStatus && window.__getGoogleSignInStatus()) || 'idle'; }")
private external fun wasmGetGoogleSignInStatus(): String

@OptIn(kotlin.js.ExperimentalWasmJsInterop::class)
@JsFun("() => { return (window.__getGoogleSignInToken && window.__getGoogleSignInToken()) || ''; }")
private external fun wasmGetGoogleSignInToken(): String

@OptIn(kotlin.js.ExperimentalWasmJsInterop::class)
@JsFun("() => { return (window.__getGoogleSignInError && window.__getGoogleSignInError()) || ''; }")
private external fun wasmGetGoogleSignInError(): String

class WasmJsGoogleAuthProvider : GoogleAuthProvider {
    override suspend fun signIn(): GoogleAuthResult {
        wasmEnsureJsHelper()
        wasmStartGoogleSignIn(GoogleAuthConfig.WEB_CLIENT_ID)
        while (true) {
            delay(150)
            when (val status = wasmGetGoogleSignInStatus()) {
                "success" -> {
                    val token = wasmGetGoogleSignInToken()
                ...
                "error" -> {
                    val err = wasmGetGoogleSignInError()
...
actual fun rememberGoogleAuthProvider(): GoogleAuthProvider {
    return remember { WasmJsGoogleAuthProvider() }
}
